...
`waxal-w2vbert-lin-specialist` is a single-language Lingala CTC model, and its frontmatter language
list is `[ln]` for that reason. It continues the `waxal-w2vbert-linsnalug-raw` checkpoint, itself a
...
the CTC head keeps its token order and stays averageable with its siblings. Training is a two-stage
curriculum: AfriVoice Lingala first, then a refresh on WAXAL Lingala gold alone. In the final system
this model is the Lingala branch of a language-identification-routed pair and one of the ensemble
members, and it produced the Lingala pseudo-labels used in a later experiment.

## Training data

| stage | source | rows | hours | licence |
|---|---|---|---|---|
| 1 | [KasuleTrevor/Lingala_100hrs](https://huggingface.co/datasets/KasuleTrevor/Lingala_100hrs), AfriVoice Lingala | 22,131 | 104.5 | CC-BY-4.0 |
| 1 and 2 | [google/WaxalNLP](https://huggingface.co/datasets/google/WaxalNLP), Lingala train and validation | not measured separately | not measured separately | CC-BY-4.0 |

The WAXAL Lingala and Shona train and validation splits are recorded jointly, as 33,002 rows and
about 179 hours, with no per-language breakdown, so no Lingala-only row count or duration is asserted
here. The speaker-disjoint holdout and the validation rows are removed before training. Stage 1 sees
both sources in every batch; stage 2 sees WAXAL Lingala only.
...
| corpus | WAXAL Lingala plus AfriVoice Lingala | WAXAL Lingala only |
| epochs | 3 | 1 |
...
gold last scored 0.2785. The mechanism is transcription convention, not acoustics: AfriVoice
transcripts carry less punctuation than WAXAL's, so a model trained on them drifts away from the
reference conventions and the refresh epoch pulls it back. Where the external corpus is already
punctuated the refresh buys nothing, which was also measured on a FLEURS-supplemented arm (0.2809
before the refresh, 0.2812 after).

## Evaluation

| split | error, 0.5 * (WER + CER) |
|---|---|
| holdout, Lingala | 0.2785 |
...
No solo public-leaderboard score was recorded for this checkpoint on its own. Routed with its Shona
...
Self-training on the WAXAL unlabeled Lingala pool, using this model's own pseudo-labels (22,986 rows,
top 60 per cent kept), was flat in both languages and was dropped.

## Intended use and limitations

Intended use is automatic speech recognition for Lingala photo-description speech: spontaneous
descriptions of images, on consumer recording equipment, averaging 20.2 seconds per clip. It is a
specialist and should be given Lingala audio only. Its behaviour on Shona, on Luganda, or on any other
...
MODEL_ID = "anyantudre/waxal-w2vbert-lin-specialist"   # or a local checkpoint directory
...
wav, sr = sf.read("lingala_clip.wav")   # mono, resample to 16 kHz before this call
